...
export default class Ride {
    constructor (
        ...
        if (fromLong < -180 || fromLong > 180) throw new Error('The latitude is invalid');
        if (toLong < -180 || toLong > 180) throw new Error('The latitude is invalid');
    }
    ...
    calculateDistance (): number {
        ...
        const a = Math.sin(deltaLat / 2) * Math.sin(deltaLat / 2)
            ...
            * Math.cos(this.toLong * degreesToRadians)
